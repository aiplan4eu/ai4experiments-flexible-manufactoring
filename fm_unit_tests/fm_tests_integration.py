...
class TestFMProcessIntegration(unittest.TestCase):
    ...
    def test_to_heat_treated_gear(self):
        self.problem.set_initial_value(buff_start(family_F1, piece_gear), 3000)
        self.problem.set_initial_value(expected_final_output(family_F1), 1)
        self.problem.add_goal(GE(stock_after(step_heat_treatment, family_F1, piece_gear), expected_final_output(family_F1)))
        ...
        with OneshotPlanner(name=self.planner_name, params={"heuristic" : self.heuristic}) as planner:
            ...
            assert(result.status == up.engines.PlanGenerationResultStatus.SOLVED_SATISFICING)
            print("{0}".format(result.plan))
            assert(len(result.plan.timed_actions) == 3)

    def test_to_heat_treated_pinion(self):
        self.problem.set_initial_value(buff_start(family_F1, piece_pinion), 3000)
        self.problem.set_initial_value(expected_final_output(family_F1), 1)
        self.problem.add_goal(GE(stock_after(step_heat_treatment, family_F1, piece_pinion), expected_final_output(family_F1)))
    ...
    def test_from_heat_treated(self):
        self.problem.set_initial_value(stock_after(step_heat_treatment, family_F1, piece_pinion), 3000)
        self.problem.set_initial_value(stock_after(step_heat_treatment, family_F1, piece_gear), 3000)
        self.problem.set_initial_value(expected_final_output(family_F1), 1)
    ...
    def test_full_process_multiple_families(self):
        self.problem.set_initial_value(buff_start(family_F3, piece_pinion), 3000)
        self.problem.set_initial_value(buff_start(family_F3, piece_gear), 3000)
        self.problem.set_initial_value(expected_final_output(family_F3), 1)
        ...
        #self.problem.add_goal(GE(stock_after(step_cutting, family_F1, piece_pinion), expected_final_output(family_F1)))
        #self.problem.add_goal(GE(stock_after(step_cutting, family_F1, piece_gear), expected_final_output(family_F1)))
        # self.problem.add_goal(GE(stock_after(step_cutting, family_F3, piece_pinion), expected_final_output(family_F3)))
        # self.problem.add_goal(GE(stock_after(step_cutting, family_F3, piece_gear), expected_final_output(family_F3)))

        self.problem.add_goal(GE(stock_lapped(family_F1), expected_final_output(family_F1)))
        self.problem.add_goal(GE(stock_lapped(family_F3), expected_final_output(family_F3)))
